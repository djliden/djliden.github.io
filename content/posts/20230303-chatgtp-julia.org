#+TITLE: Using the ChatGTP API with Julia Part 1: the HTTP.jl Library
#+DATE: <2023-03-04 Sat>
...
with the OpenAI ChatGTP API, which was made public a few days ago. This post
...
# #+begin_src jupyter-julia :eval no-export :results silent
# using Pkg
...
:header-args:jupyter-julia: :session chatgtp :display plain :async yes :kernel julia-1.8
:END:
One of the things that makes ChatGTP useful is its ability to "remember" past
...
2. ~user~: The human interacting with ChatGTP.
3. ~assistant~: the responses from ChatGTP
...
This post showed basic usage of the ChatGTP API with Julia. In the next post,
