...
#+ATTR_HTML: :alt Claude Desktop Developer settings showing the running SecretServer MCP server and an Edit Config button
...
#+ATTR_HTML: :alt Claude Desktop chat box with the tools and MCP connector icons highlighted below the message field
...
#+ATTR_HTML: :alt Claude Desktop 'Available MCP tools' popup listing get_secret_number from SecretServer
...
#+ATTR_HTML: :alt Claude Desktop dialog asking permission to run get_secret_number from SecretServer, with Allow and Deny options
[[./figures/20250412-mcp-1/4-claude-permission.png]]

Now Claude can use the tool to retrieve the secret number.

#+ATTR_HTML: :alt Claude Desktop replying 'The secret number is 13' after calling the get_secret_number tool
