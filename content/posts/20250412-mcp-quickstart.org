...
To get started, we will set up an MCP server that exposes a single function, ~get_secret_number~, that simply returns a "secret number" to the LLM. When you ask an LLM with this capabillity for the secret number, it will invoke the tool and return the "secret number," verifying that it successfully used the MCP server.
