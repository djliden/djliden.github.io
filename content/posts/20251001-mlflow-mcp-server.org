...
#+ATTR_HTML: :alt MLflow UI trace detail with nested spans summarize_content, scrape_webpage, and Completions plus chat messages
...
2. From the *Settings* menu, click *Developer* and then *Edit Config*:
    ...
    #+ATTR_HTML: :alt Claude Desktop Developer settings Local MCP servers pane with the Edit Config button highlighted
...
4. *Restart Claude Desktop.* After restarting, you should see that ~mlflow-mcp~ appears in the Claude Desktop connections menu:
    ...
    #+ATTR_HTML: :alt Claude Desktop connections menu with the mlflow-mcp server toggled on and highlighted
...
5. *Try it out!*
   ...
   Claude called the ~Search traces~ and ~get trace~ functions to identify the relevant trace, and then responded with a diagnosis of the issue and suggested next steps:
    ...
    #+ATTR_HTML: :alt Claude Desktop diagnosing a failed trace, explaining the gpt-nonexistent-model 404 error with recommended fixes
...
Look at the traces in experiment 697822894089422973. Find the most recent one that had a retrieval component and tell me what was retrieved and whether the retrieval was successful.
   #+end_quote
    ...
    #+ATTR_HTML: :alt Claude Code terminal calling mlflow-mcp search_traces and reporting a successful webpage retrieval from a trace
