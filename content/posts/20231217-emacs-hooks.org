...
#+AUTHOR: Daniel Liden
#+DATE: <2023-12-17 Sun>
#+DESCRIPTION: An introduction to Emacs hooks: what they do, how they work, and how to use mode hooks and use-package to run code at the right time.
#+KEYWORDS: Emacs, hooks, Emacs Lisp, use-package, configuration
* Emacs Hooks
#+begin_preview
...
#+CAPTION: Invoking ~org-tree-slide-mode~ runs ~org-tree-slide-play-hook~, a list of functions meant to run when a slide show is started in ~org-tree-slide-mode~. In this case, the hook makes the text larger and centered in the buffer.
 #+ATTR_HTML: :alt Animation of org-tree-slide-mode making an Emacs org buffer's text larger and centered via a play hook
...
#+CAPTION: Diagram of the basic process of working with normal hooks
 #+ATTR_HTML: :alt Hand-drawn diagram: add-hook adds no-argument hook functions to a hook variable that run-hooks calls in sequence
